...
def PrimaryCap(inputs, dim_capsule, n_channels, kernel_size, strides, padding):
    ...
    # For dim_capsule=8, n_channels=32:
    #  * output is tensor with shape [None, 6, 6, 32*8] where None represents the batch size.
    #  * Reshaping does NOT modify the batch size (leading dimension).
    #    It reshapes [6, 6, 32*8] to [6*6*32, 8] = [1152, 8]
    # When initializing the reshaping layer independently and computing its output shape
    #         _r = layers.Reshape(target_shape=[-1, dim_capsule])
    #         _r.compute_output_shape(output.shape)
    # it returns [None, 1152, 8].
    # Strangely, when applying _r to output, the returned tensor has an unknown dimension at index 1:
    #         _r(output).get_shape()
